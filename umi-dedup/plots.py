...
def main():
    ...
    x = range(len(counts))
    ax[0].bar([i - 0.2 for i in x], counts.raw_reads, width=0.4,
              label="raw reads", color=PALETTE["grey"],
              edgecolor=OUTLINE, linewidth=OUTLINE_WIDTH)
    ax[0].bar([i + 0.2 for i in x], counts.collapsed_umi, width=0.4,
              label="collapsed UMIs", color=PALETTE["blue"],
              edgecolor=OUTLINE, linewidth=OUTLINE_WIDTH)
    ...
    ax[1].plot(sat.reads, sat.molecules, marker="o", color=PALETTE["rose"],
               markeredgecolor=OUTLINE, markeredgewidth=OUTLINE_WIDTH)
    ax[1].set(title="Library saturation", xlabel="reads sampled", ylabel="unique molecules")
    finalize(fig)
    out = ASSETS / "umi_qc.png"; fig.savefig(out, dpi=130)
    print("wrote", out)
